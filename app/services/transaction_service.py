...
class TransactionService:
    ...
    async def create_prolong_proxy_transaction(
            self, user: User, amount: float, new_balance: float, related_ids: int, provider: str | None) -> dict:
        if not user or not user.balance:
            return {
                "success": False,
                "status_code": 404,
        ...
        transaction = Transaction(
            ...
            type="prolong",
            status="pending",
            comment=f"Prolong proxies for user {user.id}"
        )
